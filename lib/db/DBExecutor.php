...
class DBExecutor
{
    ...
    public static function GetPosts(string $user, int $quan, int $offset): array
    {
        ...
        return self::getConn()->multirows('SELECT phid, path, description, addtime FROM photo WHERE ownerLogin=:owner ORDER BY addtime DESC LIMIT :quan OFFSET :offset', ['owner' => $user, 'quan' => $quan, 'offset' => $offset]);
    }
    ...
    public static function AddPhoto(string $login, string $path, string $description, \DateTime $dt): int
    {
        ...
        return self::getConn()->nonQuery('INSERT INTO photo VALUES (default, :path, :owner, :descr, :addedtime)', ['path' => $path, 'owner' => $login, 'descr' => $description, 'addedtime' => $timestr]);
    }
    ...
    {
        return self::getConn()->row('SELECT AVG(value) as avgmarks, COUNT(value) as countmarks from mark WHERE phId=:phId', ['phId' => $phId]);
    }

    public static  function GetMarksByUser(string $phId, string $login): int
    {
        return self::getConn()->scalar('SELECT value FROM mark WHERE userId=:user AND phId=:photo', ['user' => $login, 'photo' => intval($phId)]);
    }

    public static function SetMark(string $postId, string $userId, int $value): int
    {
        return self::getConn()->nonQuery('INSERT INTO mark VALUES (:userId, :phId, :value)', ['userId' => $userId, 'phId' => $postId, 'value' => $value]);
    }
    ...
    public static function GetPostsofSubscriptions(string $userId, int $count, $offset): array
    {
        return self::getConn()->multirows('SELECT phid, path, description, addtime, ownerlogin FROM photo WHERE ownerlogin IN (
            SELECT login FROM subs WHERE sublogin = :userId) ORDER BY addtime DESC LIMIT :quan OFFSET :offset', ['userId' => $userId, 'quan' => $count, 'offset' => $offset]);
    }
}
